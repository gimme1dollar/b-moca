...
from bmoca.environment.wrapper import GeminiActionParsingWrapper

from bmoca.agent.foundation_model.gemini import load_model
from bmoca.agent.foundation_model.gemini import build_prompt, build_few_shot_prompt
...
def step(args, env,
    ...
    # predict action
    if args.multi_modal:
        raw_act = model.generate_content([prompt, imgpil]).text
    else:
...
def evaluate_one_env(args, \
    ...
    env = GeminiActionParsingWrapper(env)
...
def parse_args():
    ...
    # logging
    parser.add_argument('--log_dir', type=str, default=f"{_WORK_PATH}/logs/gemini")
