...
def build_few_shot_prompt(few_shot_examples, k=1, trajectory_type="trajectory"):
    expert_demonstration = ""
    ...
    if trajectory_type == "trajectory":
        ...
        return re.sub(
            r"<expert_demonstration>", expert_demonstration, base_prompt.FEW_SHOT_PROMPT
        )

    elif trajectory_type == "transition":
        # NOTE: while each example is not actually transition (just obs-act pair),
        # termining it in this way has shown better performance
        ...
        return re.sub(r"<expert_demonstration>", expert_demonstration, FEW_SHOT_PROMPT)
